...
| API integration | Vitest + Testcontainers (Postgres/Redis)    | Module services against real DB; tenant-isolation suite (cross-store access must fail); idempotency replay suite. _Phase 0 note (approved 2026-07-09): tenant-isolation + identity suites run on PGlite (real Postgres in WASM — RLS/roles fully exercised, no Docker needed locally or in CI); Testcontainers arrives with the Phase 1 integration suites._ |
